#!/usr/bin/env python3
"""
verify_result.py — batched OFDM mod kernel 端到端验证

比较 kernel 输出 (int16, AIR_SCALE=1600, separated re/im, 30720 时域样点, 含 CP)
...
OUTPUT_MODE = os.environ.get("OFDM_OUTPUT_MODE", "all")
...
def main():
    kernel_paths = (K_IQ,) if OUTPUT_MODE == "iq" else (K_RE, K_IM)
    for p in (G_RE, G_IM, *kernel_paths):
    ...
    if OUTPUT_MODE == "iq":
        iq_raw = np.fromfile(K_IQ, dtype=np.int16)
        k_re = iq_raw[0::2].astype(np.float32) / AIR_SCALE
        k_im = iq_raw[1::2].astype(np.float32) / AIR_SCALE
    else:
    ...
    print(f"[verify] kernel output mode = {OUTPUT_MODE}")
    total_samples = BATCH_SIZE * N_SAMPLE
    print(f"[verify] batch={BATCH_SIZE} samples/slot={N_SAMPLE} tolerance={ABS_TOL}\n")

    for name, k, g in (("re", k_re, g_re), ("im", k_im, g_im)):
        if k.size != total_samples or g.size != total_samples:
            print(f"[verify] {name} size mismatch: kernel={k.size} golden={g.size} (want {total_samples})")
            sys.exit(1)

    g_re = g_re.reshape(BATCH_SIZE, N_SAMPLE)
    g_im = g_im.reshape(BATCH_SIZE, N_SAMPLE)
    k_re = k_re.reshape(BATCH_SIZE, N_SAMPLE)
    k_im = k_im.reshape(BATCH_SIZE, N_SAMPLE)

    # 逐 symbol body 区间比较 (CP 区间与 body 尾部相同, 一起覆盖)
    offs = np.concatenate([[0], np.cumsum(CP_LENS + N_FFT)])
    print("[verify] per-batch maximum body |err|:")
    overall_max = 0.0
    n_pass = 0
    for b in range(BATCH_SIZE):
        ...
        for s in range(N_SYMBOL):
            ...
            er = float(np.abs(k_re[b, sl] - g_re[b, sl]).max())
            ei = float(np.abs(k_im[b, sl] - g_im[b, sl]).max())
            m = max(er, ei)
            batch_max = max(batch_max, m)
            overall_max = max(overall_max, m)
            n_pass += int(m <= ABS_TOL)
        print(f"  batch {b:2d}: max={batch_max:7.4f} [{'OK' if batch_max <= ABS_TOL else 'FAIL'}]")
    ...
    if OUTPUT_MODE == "iq":
        ...
        iq_ok = iq_i.size == 2 * total_samples
        print(f"[verify] IQ-only stream validated against golden tolerance   "
              f"[{'OK' if iq_ok else 'FAIL'}]")
    elif OUTPUT_MODE == "all" and os.path.exists(K_IQ):
        kr_i = np.fromfile(K_RE, dtype=np.int16)
        ki_i = np.fromfile(K_IM, dtype=np.int16)
        iq_i = np.fromfile(K_IQ, dtype=np.int16)
        if iq_i.size != 2 * total_samples:
            print(f"[verify] IQ size mismatch: {iq_i.size} (want {2*total_samples})")
            iq_ok = False
        else:
        ...
        print(f"[verify] IQ check skipped for output mode {OUTPUT_MODE}")
    print()

    expected = BATCH_SIZE * N_SYMBOL
    if n_pass == expected and cp_ok and iq_ok:
        print(f"[verify] ========== PASS ({n_pass}/{expected}, CP OK, IQ OK) ==========")
        return 0
    print(f"[verify] ========== FAIL ({n_pass}/{expected}, CP {'OK' if cp_ok else 'FAIL'}, IQ {'OK' if iq_ok else 'FAIL'}) ==========")
    return 1
