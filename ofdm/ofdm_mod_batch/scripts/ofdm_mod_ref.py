"""
Batched OFDMModulator reference for 5G NR (30 kHz SCS, 14-symbol slot).
...
_DATA_OVERRIDE = Path(os.environ["AIRAN_DATA_DIR"]) if os.environ.get("AIRAN_DATA_DIR") else None
DEFAULT_WEIGHTS_DIR = ((_DATA_OVERRIDE / "weights" / "ofdm")
                       if _DATA_OVERRIDE else (_KERNEL_DIR / "weights"))
DEFAULT_GOLDEN_DIR  = ((_DATA_OVERRIDE / "ofdm_golden")
                       if _DATA_OVERRIDE else (_KERNEL_DIR / "data" / "golden"))
...
def verify_against_sionna(S4_grid, x_bodies, atol=1.0):
    ...
    Xnat = np.zeros((*S4_grid.shape[:-2], N_FFT), np.complex64)
    for b1 in range(P):
        for b2 in range(Q):
    ...
    try:
        ...
        try:
            import torch
            y = mod(torch.tensor(Xc, dtype=torch.complex64)).detach().cpu().numpy()
        except ImportError as e:
...
def dump_goldens(S4_grid, x_bodies, out_root=None, weights_root=None):
    ...
    S4_flat = S4_grid.reshape(BATCH_SIZE, N_SYMBOL_PER_SLOT, N_FFT)
    ...
    S4_q = (re16.astype(np.float32) + 1j * im16.astype(np.float32)).reshape(
        BATCH_SIZE, N_SYMBOL_PER_SLOT, P, Q)
    ...
    iq_i = np.empty(2 * re_i.size, dtype=np.int16)
    iq_i[0::2] = re_i.reshape(-1)
    iq_i[1::2] = im_i.reshape(-1)
    ...
    iW64r, iW64i = IW_DFT_Q.real.astype(np.float32), IW_DFT_Q.imag.astype(np.float32)
    iTr,   iTi   = IT_PQ.real.astype(np.float32),    IT_PQ.imag.astype(np.float32)
    Ar, Ai = cmm(Sr, Si, iW64r, iW64i, "...ab,bc->...ac")
    Br = Ar * iTr - Ai * iTi; Bi = Ar * iTi + Ai * iTr
    Cr, Ci = cmm(iW32r, iW32i, Br, Bi, "ab,...bc->...ac")
...
if __name__ == "__main__":
    print("=" * 60)
    print(" Batched OFDM Modulator — 严格逆变换数据生成 + 自检")
    print("=" * 60)
    print(f"  KERNEL_DIR  = {_KERNEL_DIR}  BATCH_SIZE = {BATCH_SIZE}\n")
    ...
    x_unit = ((rng.standard_normal((BATCH_SIZE, N_SYMBOL_PER_SLOT, N_FFT))
               + 1j * rng.standard_normal((BATCH_SIZE, N_SYMBOL_PER_SLOT, N_FFT)))
