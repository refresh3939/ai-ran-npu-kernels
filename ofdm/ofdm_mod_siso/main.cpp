...
extern "C" void ofdm_mod_kernel(uint8_t *, uint8_t *, uint8_t *, uint8_t *, uint8_t *,
...
int32_t main(int32_t  , char *  [])
{
    ...
    const size_t inHalfBytes  = static_cast<size_t>(N_SYMBOL) * N_FFT * sizeof(int16_t);
    const size_t outBytes     = static_cast<size_t>(N_SAMPLE_PER_SLOT) * sizeof(int16_t);
    const size_t iqBytes      = static_cast<size_t>(2 * N_SAMPLE_PER_SLOT) * sizeof(int16_t);
    ...
    printf("[ofdm_mod] SOC=%s blockDim=%u N_FFT=%u symbols=%u\n",
           socVersion, blockDim, N_FFT, N_SYMBOL);
    printf("[ofdm_mod] scratch: %zu KB per core\n", scratchPerCoreBytes / 1024);
    ...
    ICPU_RUN_KF(ofdm_mod_kernel, blockDim,
                inRe, inIm, w32r, w32i, w64r, w64i, twr, twi, scr,
                outRe, outIm, outIq, ws, til);
    ...
    printf("[ofdm_mod] warm-up: %d runs\n", N_WARMUP);
    for (int i = 0; i < N_WARMUP; ++i) {
        CHECK_ACL(aclrtMemset(scrD, scratchBytes, 0, scratchBytes));
        ACLRT_LAUNCH_KERNEL(ofdm_mod_kernel)
        (blockDim, stream,
         inReD, inImD, w32rD, w32iD, w64rD, w64iD, twrD, twiD, scrD,
         outReD, outImD, outIqD, wsD, tilD);
        CHECK_ACL(aclrtSynchronizeStream(stream));
    }

    printf("[ofdm_mod] timed:   %d runs\n", N_TIMED);
    ...
    printf("[ofdm_mod] latency: min %.1f us, avg %.1f us, p50 %.1f us, p99 %.1f us, max %.1f us\n",
           min_us, avg_us, p50, p99, max_us);
    printf("[ofdm_mod] real-time headroom: %.1fx\n", 1000.0 / avg_us);
