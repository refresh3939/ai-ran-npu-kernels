#!/bin/bash
# channel_est_ls v5 — build + run + verify
set -e
...
echo "================================================="
echo "[run.sh]  channel_est_ls v5 (fp16 4-stream)"
...
if [ ! -d "${GOLDEN_DIR}" ] || [ ! -f "${GOLDEN_DIR}/case_0_awgn_only/input.bin" ]; then
    echo "[run.sh] golden 数据不存在,先跑:"
    echo "         python3 scripts/channel_est_ref.py"
    exit 1
fi
