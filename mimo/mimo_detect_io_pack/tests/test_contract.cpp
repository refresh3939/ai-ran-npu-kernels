...
pack::MimoDetectLayerPlan MakePlan(
    const std::array<pack::PuschMimoConfig, 3> &configs)
{
...
{
    const std::array<pack::PuschMimoConfig, 3> configs = {
        MakeConfig(4, 4), MakeConfig(3, 4), MakeConfig(2, 2)};
    ...
    ok &= Check(pack::BuildCurrentProfile(configs.data(), configs.size(), plan,
                                          &metadata) == pack::OK,
                "valid mixed-rank plan rejected");
    ok &= Check(metadata.magic == pack::META_MAGIC && metadata.active_layers == 9 &&
    ...
    bad = plan;
    bad.allocations[3].num_layers = 1;
    ok &= Check(pack::BuildCurrentProfile(configs.data(), configs.size(), bad,
                                          &metadata) == pack::PLAN_MISMATCH,
                "non-zero unused allocation was accepted");

    auto bad_configs = configs;
    bad_configs[1].slot_number = 8;
    ok &= Check(pack::BuildCurrentProfile(bad_configs.data(), bad_configs.size(), plan,
                                          &metadata) == pack::PLAN_MISMATCH,
                "mixed detection regions were accepted");
    bad_configs = configs;
    bad_configs[2].num_rx_antennas = pack::NR == 64 ? 32 : 64;
    ok &= Check(pack::BuildCurrentProfile(bad_configs.data(), bad_configs.size(), plan,
    ...
    std::printf("[contract] %s: mixed ranks [4,3,2], offsets [0,4,7], activeL=9\n",
                ok ? "PASS" : "FAIL");
    return ok ? 0 : 1;
}
