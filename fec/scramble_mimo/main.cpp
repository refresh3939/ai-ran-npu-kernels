...
using namespace airan::scramble_mimo;
...
double RunKernel(const ScrambleMimoOpArgsV1 &args, const Buffer &gold,
                 Buffer &workspace, const Buffer &tiling)
{
    auto enqueue = [&]() {
        const Status status = Enqueue(args, gold.device, gold.bytes,
             ...
             Buffer &workspace)
{
    ...
    if (BuildCurrentProfile(config, num_slots, &layout, &metadata) != OK) {
    ...
    Buffer input;
    input.Load(golden_dir + "/bits_nr.bin");
    if (input.bytes != bytes) {
        std::fprintf(stderr, "[FAIL] %s input size mismatch\n", name.c_str());
        std::exit(EXIT_FAILURE);
    }

    Buffer gold;
    gold.Allocate(bytes);
    if (BuildGoldBits(config, layout, num_slots,
                      static_cast<int16_t *>(gold.host), elements) != OK) {
        std::fprintf(stderr, "[FAIL] BuildGoldBits failed for %s\n", name.c_str());
        std::exit(EXIT_FAILURE);
    }
    const auto expected_gold = LoadHostI16(golden_dir + "/gold.bin", elements);
    if (!std::equal(expected_gold.begin(), expected_gold.end(),
                    static_cast<const int16_t *>(gold.host))) {
        std::fprintf(stderr, "[FAIL] C++ Gold disagrees with Python for %s\n",
                     name.c_str());
        std::exit(EXIT_FAILURE);
    }
    ACL_CHECK(aclrtMemcpy(gold.device, gold.bytes, gold.host, gold.bytes,
                         ACL_MEMCPY_HOST_TO_DEVICE));

    const auto expected = LoadHostI16(golden_dir + "/bits_qam.bin", elements);
    std::vector<int16_t> reference(elements);
    if (ReferenceScramble(static_cast<const int16_t *>(input.host),
                          static_cast<const int16_t *>(gold.host), config, layout,
                          num_slots, reference.data()) != OK || reference != expected) {
        std::fprintf(stderr, "[FAIL] C++ reference disagrees with Python for %s\n",
                     name.c_str());
        std::exit(EXIT_FAILURE);
    }
    ...
    ScrambleMimoOpArgsV1 args {};
    args.abi_version = ABI_VERSION;
    args.struct_size = sizeof(args);
    args.bits_nr = input.device;
    args.bits_qam = output.device;
    args.config = &config;
    args.layout = &layout;
    ...
    const double latency_us = RunKernel(args, gold, workspace, tiling);
    output.Save(output_dir + "/bits_qam.bin");
    std::printf("[case] %-5s slots=%u shape=[%u,8,%u] launches=1 cores=%u "
                "p50=%.1f us (%.2f us/slot)\n",
                name.c_str(), num_slots, num_slots, layout.codeword_stride,
                BLOCK_DIM, latency_us, latency_us / num_slots);
}
