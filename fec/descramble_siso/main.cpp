...
static std::string data_dir() {
    ...
    std::string base = e ? std::string(e) : "../../..";
    return base + "/data/descramble";
}
...
int main() {
    ...
    printf("=== descramble test ===\n");
    printf("N_SLOT=%u  input=[8,%u,%u]  output=[8,%u] compact\n",
           N_SLOT, N_DATA_SYM, QAM_SYM_STRIDE, N_SYM_PAD);
    printf("IN_BYTES=%.2f MB  OUT_BYTES=%.2f MB\n", IN_BYTES/1e6, OUT_BYTES/1e6);
    printf("data dir: %s\n", ddir.c_str());


    auto h_input  = load_bin<int16_t>(ddir + "/input.bin",     IN_ELEMS);
    auto h_sign   = load_bin<int16_t>(ddir + "/sign_flat.bin", IN_ELEMS);
    auto h_golden = load_bin<int16_t>(ddir + "/golden.bin",    OUT_ELEMS);
    printf("Loaded: input=%zu  sign=%zu  golden=%zu  elements\n",
           h_input.size(), h_sign.size(), h_golden.size());
    ...
    void *d_input, *d_sign, *d_nslot, *d_output;
    ACL_CHECK(aclrtMalloc(&d_input,  IN_BYTES,   ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMalloc(&d_sign,   IN_BYTES,   ACL_MEM_MALLOC_HUGE_FIRST));
    ...
    void *d_ws = nullptr, *d_tiling = nullptr;
    ...
    ACL_CHECK(aclrtMemcpy(d_sign,   IN_BYTES,   h_sign.data(),  IN_BYTES,  ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_CHECK(aclrtMemcpy(d_nslot,  NSLOT_BYTES, h_nslot.data(), NSLOT_BYTES, ACL_MEMCPY_HOST_TO_DEVICE));


    ACLRT_LAUNCH_KERNEL(descramble_kernel)(BLOCK_DIM, stream, d_input, d_sign, d_nslot, d_output, d_ws, d_tiling);
    ...
    if (first_err >= 0) {
        int slot = first_err / (N_SYM_PAD * N_STREAMS);
        int b    = (first_err / N_SYM_PAD) % N_STREAMS;
        int re   = first_err % N_SYM_PAD;
        printf("  First error: flat=%d slot=%d re=%d b=%d  got=%d golden=%d\n",
               first_err, slot, re, b, (int)h_output[first_err], (int)h_golden[first_err]);
    }
    ...
    aclrtFree(d_input); aclrtFree(d_sign); aclrtFree(d_nslot); aclrtFree(d_output);
