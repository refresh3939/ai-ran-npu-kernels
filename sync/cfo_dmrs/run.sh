#!/bin/bash
# cfo_dmrs v2 — DMRS-based CFO tracker (Layer 3): build + run + verify
set -e
...
echo "================================================="
echo "[run.sh]  cfo_dmrs v2 (Layer 3 DMRS-based CFO tracker)"
...
# 生成 golden(Phase 0 ref) — v2 探测 4-plane 输入 y_re.bin
if [ ! -f "${AIRAN_DATA_DIR}/data/golden/case_0_no_cfo/y_re.bin" ]; then
    echo "[run.sh] generating golden via cfo_dmrs_ref.py ..."
    python3 ${SCRIPT_DIR}/scripts/cfo_dmrs_ref.py
fi
