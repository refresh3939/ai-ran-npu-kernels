#!/bin/bash
# pss_cfo_estimator — PSS-based fractional CFO estimator: build + run + verify
set -e
...
echo "================================================="
echo "[run.sh]  pss_cfo_estimator (PSS-based fractional CFO)"
...
# 生成 golden (Phase 0 ref) — 如果还没跑过
if [ ! -f "${AIRAN_DATA_DIR}/data/golden/case_0_no_cfo/input.bin" ]; then
    echo "[run.sh] generating golden via pss_cfo_estimator_ref.py ..."
    python3 ${SCRIPT_DIR}/scripts/pss_cfo_estimator_ref.py
fi
mkdir -p ${AIRAN_DATA_DIR}/data/ascend_output

# 抑制 INFO log spam (SKILL/memory 规则)
