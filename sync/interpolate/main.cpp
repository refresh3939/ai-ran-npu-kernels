...
#include "acl/acl.h"
#include "aclrtlaunch_interpolate_kernel.h"
#else
#include "tikicpulib.h"
extern "C" void interpolate_kernel(uint8_t *, uint8_t *, uint8_t *, uint8_t *, uint8_t *);
#endif
...
int32_t main(int32_t, char *[])
{
    ...
    const size_t tilingBytes = static_cast<size_t>(IDX_GM_LEN) * sizeof(uint32_t);
    ...
    printf("[interpolate] SOC=%s blockDim=%u  N_IN=%u N_OUT=%u  L_TAP=%u\n",
    ...
    ICPU_RUN_KF(interpolate_kernel, blockDim, inp, taps, outIQ, ws, til);
    ...
    printf("[interpolate] warm-up: %d\n", N_WARMUP);
    for (int i = 0; i < N_WARMUP; ++i) {
        ACLRT_LAUNCH_KERNEL(interpolate_kernel)
        (blockDim, stream, inpD, tapsD, outD, wsD, tilD);
        CHECK_ACL(aclrtSynchronizeStream(stream));
    }

    printf("[interpolate] timed: %d\n", N_TIMED);
    ...
    printf("[interpolate] latency: min %.1f us, avg %.1f us, p50 %.1f us, max %.1f us\n",
