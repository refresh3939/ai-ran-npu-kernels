...
echo "================================================="
echo "[run.sh]  cfo_estimate"
echo "[run.sh]  RUN_MODE=${RUN_MODE}  SOC=${SOC_VERSION}"
echo "================================================="

# AIRAN_DATA_DIR 指向 kernel 目录 (本地化 data/)
...
if [ ! -d "${GOLDEN_DIR}" ] || [ ! -f "${GOLDEN_DIR}/case_0_cfo_+0hz/input.bin" ]; then
    echo "[run.sh] golden 数据不存在,先跑:"
    echo "         python3 scripts/cfo_estimate_ref.py"
    exit 1
fi
