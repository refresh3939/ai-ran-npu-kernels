"""
decimate ×8 verify — kernel 交织单 buffer 输出 vs golden (都交织, 直接比)。

kernel 输出 (对外交织, drop-in 喂 pss/sss):
    data/ascend_output/output.bin     int16 IQ 交织 [2*N_OUT]
golden (decimate_ref.py):
...
def main():
    ...
    core_bounds = [38400, 76800, 115200]

    print("=" * 60)
    print(" decimate ×8 verify  (kernel 交织  vs  golden 交织)")
    print("=" * 60)
    ...
    for cb in core_bounds:
        lo, hi = max(0, cb - 4), min(N_OUT, cb + 4)
        print(f"     @{cb:<7} eI={ei[lo:hi].max():>4d} eQ={eq[lo:hi].max():>4d}")

    # 交织结构自检: 若 I/Q 整体错位 (打包写反), eI/eQ 会同时爆大
