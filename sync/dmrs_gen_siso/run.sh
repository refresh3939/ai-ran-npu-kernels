...
echo "================================================="
echo "[run.sh]  dmrs_gen (TX PUSCH DMRS generator)"
...
# 生成 golden(Phase 0 ref)
if [ ! -f "${AIRAN_DATA_DIR}/data/golden/case_0_baseline/cinit.bin" ]; then
    echo "[run.sh] generating golden via dmrs_gen_ref.py ..."
    python3 ${SCRIPT_DIR}/scripts/dmrs_gen_ref.py
fi
