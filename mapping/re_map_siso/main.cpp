...
extern "C" void re_map_kernel(uint8_t *, uint8_t *, uint8_t *, uint8_t *, uint8_t *,
                              uint8_t *, uint8_t *);
#endif
...
std::string GIdx()   { return DataDir() + "/weights/scatter_idx.bin"; }
std::string GOutRe() { return DataDir() + "/data/ascend_output/grid_re.bin"; }
std::string GOutIm() { return DataDir() + "/data/ascend_output/grid_im.bin"; }
...
int32_t main(int32_t  , char *  [])
{
    ...
    const size_t inBytes  = static_cast<size_t>(N_SYMBOL) * N_SC_PAD * sizeof(int16_t);
    const size_t outBytes = static_cast<size_t>(N_SYMBOL) * N_FFT    * sizeof(int16_t);
    const size_t idxBytes = static_cast<size_t>(N_FFT) * sizeof(uint32_t);
    const size_t dummyBytes = 1024;

    uint32_t blockDim = BLOCK_DIM;
    printf("[re_map] SOC=%s blockDim=%u N_FFT=%u N_SC_PAD=%u symbols=%u\n",
    ...
    ICPU_RUN_KF(re_map_kernel, blockDim, inRe, inIm, idx, outRe, outIm, ws, til);
    ...
    printf("[re_map] warm-up: %d runs\n", N_WARMUP);
    for (int i = 0; i < N_WARMUP; ++i) {
        ACLRT_LAUNCH_KERNEL(re_map_kernel)(blockDim, stream, inReD, inImD, idxD, outReD, outImD, wsD, tilD);
        CHECK_ACL(aclrtSynchronizeStream(stream));
    }

    printf("[re_map] timed:   %d runs\n", N_TIMED);
    ...
    printf("[re_map] latency: min %.1f us, avg %.1f us, p50 %.1f us, p99 %.1f us, max %.1f us\n",
