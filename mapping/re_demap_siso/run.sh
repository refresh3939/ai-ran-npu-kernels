...
echo "================================================="
echo "[run.sh]  re_demap   RUN_MODE=${RUN_MODE}  SOC=${SOC_VERSION}"
echo "================================================="
...
if [ ! -f "${AIRAN_DATA_DIR}/weights/gather_idx.bin" ]; then
    echo "[run.sh] 生成 golden + gather 表 ..."
    python3 ${SCRIPT_DIR}/scripts/re_demap_ref.py
fi
