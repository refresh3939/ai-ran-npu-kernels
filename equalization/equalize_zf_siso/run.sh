...
echo "================================================="
echo "[run.sh]  equalize (SISO single-tap, full-grid fp16)"
echo "[run.sh]  RUN_MODE=${RUN_MODE}  SOC=${SOC_VERSION}"
echo "================================================="

# kernel-self-contained: golden + output live under this op's own data/
export AIRAN_DATA_DIR=${SCRIPT_DIR}
echo "[run.sh]  AIRAN_DATA_DIR=${AIRAN_DATA_DIR}"

GOLDEN_DIR=${AIRAN_DATA_DIR}/data/golden
if [ ! -d "${GOLDEN_DIR}" ] || [ ! -f "${GOLDEN_DIR}/case_0_awgn_only/in_y_re.bin" ]; then
    echo "[run.sh] golden 数据不存在,先跑:"
    echo "         python3 scripts/equalize_ref.py"
    exit 1
fi
