...
def main() -> int:
    ...
    logical_ports = int(receipt["logical_tx_ports"])
    tx_antennas = int(receipt["tx_antennas"])
    expected_bytes = slots * tx_antennas * SAMPLES_PER_SLOT * 2 * 2

    assert receipt["schema"] == "airan.pusch_mimo.tx_chain.v1"
    assert receipt["status"] == "PASS"
    assert 1 <= rank <= 4
    assert rank <= logical_ports <= 4
    assert logical_ports <= tx_antennas <= 16
    assert 1 <= slots <= 23
    ...
    print(
        f"[PASS] fused TX Rank{rank} logical_ports={logical_ports} "
        f"tx_antennas={tx_antennas} slots={slots} "
